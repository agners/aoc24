...
def find_shortest_path(keypads: list, sequence: list[int], ) -> int:
    """Shortest path"""
    keypads = keypads.copy()
    keypad_positions: dict[int, tuple[int, int]] = keypads.pop(0)
    total_shortest_path = 0
    for paths in generate_paths_for_keypad(keypad_positions, sequence):
        shortest_path = None
        for path in paths:
            if len(keypads) == 0:
                ...
                if (len(keypads), sequence_tuple) in cache:
                    overall_path_len = cache[(len(keypads), sequence_tuple)]
                else:
                    overall_path_len = find_shortest_path(keypads, path)
                    cache[(len(keypads), sequence_tuple)] = overall_path_len
            shortest_path = min(overall_path_len, shortest_path) if shortest_path else overall_path_len
...
with open("test-data.txt") as f:
    ...
    for line in f:
        ...
        # Numeric pad...
        total_shortest_path_num = find_shortest_path(keypads, sequence)
